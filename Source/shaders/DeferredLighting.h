...
public:
	...
	void PlayerDead(bool); // State whether the player is dead.
	void InWater(bool); // True when the player head is below water
	void InsideTeleport(bool flag); // True when inside a teleport
	void SetWhitePoint(float); // The point where the HDR value shall be transformed to white.
...
private:
	...
	GLint fDeadIndex;
	GLint fHeadInWaterIdx, fInsideTeleportIdx;
	GLint fAverageLuminanceIdx;
};
